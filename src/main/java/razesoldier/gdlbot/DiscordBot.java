...
public class DiscordBot {
    ...
    public void run() {
        ...
        Mono<Void> login = client.withGateway(gateway -> {
                    Mono<Void> handleMessageCreate = gateway.on(MessageCreateEvent.class, event -> Mono.fromRunnable(new MessageCreateEventHandler(event, gdlBot, discordMsgMapQQMSg))).then();
